...
class LihatIsi4Controller extends Controller
{
    public function index(){
      $standar3 = Standar3::where("id_prodi", 1)->get();
      if($standar3->count() <= 0){
        return redirect('/standar3');
      }

      $standar="Standar 4";
      $data = Standar4::select('kode', 'data', 'skor', 'kategori')->where('id_prodi', '=', '1')->orderBy('kode', 'asc')->get();
      if(!$data->count()){
        $dataCheck = true;
      }else{
        $dataCheck = false;
    ...
    public function save(Request $request){
      ...
      $oldStandar4 = Standar4::where('id_prodi', '=', '1')->first();
      if($oldStandar4){
        //jika sudah maka ...
