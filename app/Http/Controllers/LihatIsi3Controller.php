...
class LihatIsi3Controller extends Controller
{
    public function index()
    {
      $standar2 = Standar2::where("id_prodi", 1)->get();
      if($standar2->count() <= 0){
        return redirect('/standar2');
      }

        $standar="Standar 3";
        $data = Standar3::select('kode', 'data', 'skor', 'kategori')->where('id_prodi', '=', '1')->orderBy('kode', 'asc')->get();
        if(!$data->count()){
          $dataCheck = true;
        }else{
          $dataCheck = false;
    ...
    public function save(Request $in)
    {
        ...
        $npbs=$in->n_3_3_3;
        if ($npbs>=80) {
          $kategori3_3_3=4*100/100;
        }
        elseif ($npbs<80) {
          $kategori3_3_3=5*$npbs*100/100;
        }
        ...
        $oldStandar3 = Standar3::where('id_prodi', '=', '1')->first();
        if($oldStandar3){
          //jika sudah maka ...
