...
class NilaiStandar7Controller extends Controller
{
  public function index($idprodi)
  {
    ...
    $nilai_kprodi=Standar7::select('kode','kategori')->where('id_prodi',$idprodi)->whereYear("created_at", date("Y"))->get();
    $nilai_auditor=Standar7Auditor::select('kode','kategori')->where([['id_prodi', '=',$idprodi], ['auditor_id', '=',session("auditor_id")]])->whereYear("created_at", date("Y"))->get();
