...
class LihatIsi5Controller extends Controller
{
    public function index(){
      $standar4 = Standar4::where("id_prodi", 1)->get();
      if($standar4->count() <= 0){
        return redirect('/standar4');
      }

      $standar="Standar 5";
      $data = Standar5::select('kode', 'data', 'skor', 'kategori')->where('id_prodi', '=', '1')->orderBy('kode', 'asc')->get();
      if(!$data->count()){
        $dataCheck = true;
      }else{
        $dataCheck = false;
    ...
    $dataStandar5 = Standar5::where('id_prodi', '=', '1')->first();
    if($dataStandar5) {
      // 5.1.1.a
      ...
      // 5.1.2.b
      $standar5=Standar5::where([['id_prodi','=','1'],
      ['kode','=','5.1.2.b']])->first();
