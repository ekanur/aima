...
class Authenticate_josso {
	...
	public function handle($request, Closure $next)
	{
        ...
        if(isset($user)){
            ...
            $koordinator = KProdi::where("nip","=", $user->name)->get();
            $prodi = array();
            ...
            if(sizeof($auditor)>0){
                ...
                $request->merge(array("role"=>"koordinator","nip" =>$user->name, "prodi" => json_encode($prodi)));
                Session::put("role", "koordinator");
                Session::put('nip', $user->name);
                if (null==session('id_prodi')) {
                    Session::put('id_prodi', $prodi[0]);
                }
                        // Session::put('nama_prodi', $dosen[0]->jjg_kd." ".$dosen[0]->pro_nm);
                Session::put("prodi", json_encode($prodi));
            }
